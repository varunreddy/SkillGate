...
class RetrievalConfig:
    ...
    include_instruction_chars: int = 800


class SkillRetriever:
    def __init__(self, cards: list[ExpertCard], *, use_dense: bool = False):
        self.cards = cards
    ...
    def retrieve(self, query: str, top_k: int = 3) -> list[RetrievalHit]:
        if not self.cards:
            return []
        top_k = max(1, min(int(top_k), min(20, len(self.cards))))
        ...
        if dense is None:
            ...
            hybrid = _rrf([sparse_rank, dense_rank], n_docs=len(self.cards))
