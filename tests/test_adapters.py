...
def _get_hits():
    ...
    cards = load_registry(root / "examples" / "registry" / "experts.yaml")
...
def _get_json_hits():
    root = Path(__file__).resolve().parents[1]
    cards = load_registry(root / "examples" / "registry" / "tools.enriched.json")
    retriever = SkillRetriever(cards)
...
def test_codex_adapter_contains_markdown_sections():
    ...
    assert "# Retrieved Skill Experts" in out
    assert "```md" in out
...
def test_claude_adapter_contains_xml_tags():
    ...
    assert "<retrieved_experts>" in out
